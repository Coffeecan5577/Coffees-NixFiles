{ inputs, config, ...}:
{
  flake.nixosConfigurations."Coffees-Pi-Sec-Nix" = inputs.nixpkgs.lib.nixosSystem {
    ...
    modules = [
      ...
      # Applications
      config.flake.modules.nixos.applications-actual
      config.flake.modules.nixos.applications-traefik
      ...
      config.flake.modules.nixos.security-pki
      ...
      (
        { pkgs, ... }:
        {
          ...
          networking.hostName = "Coffees-Pi-Sec-Nix";
